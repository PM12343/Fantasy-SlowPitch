...
with tab_browser:
    ...
    col_add_my, col_add_opp, _ = st.columns(3)
    with col_add_my:
...
with tab_h2h:
        ...
        if st.button("➕ Add to OPPONENT H2H Team", type="secondary",
            ...
            for p in to_add:
                if len(st.session_state.opponent_team) < 10 and p not in st.session_state.opponent_team:
                    ...
                    added += 1
            save_roster(OPPONENT_H2H_FILE, st.session_state.opponent_team)
            st.toast(f"Added {added} to opponent H2H team", icon="✅")
            st.rerun()
    ...
    with col_left:
        ...
        if st.session_state.h2h_my_team:
            ...
            for i, p in enumerate(st.session_state.h2h_my_team):
                if st.button("🗑️ Remove", key=f"h2h_my_remove_{i}"):
    ...
    with col_right:
        st.subheader("Opponent H2H Team")
        if st.session_state.opponent_team:
            ...
            for i, p in enumerate(st.session_state.opponent_team):
                if st.button("🗑️ Remove", key=f"opp_h2h_remove_{i}"):
    ...
    if st.button("🔥 Compare H2H Teams", type="primary", use_container_width=True, key="compare_h2h"):
        if len(st.session_state.h2h_my_team) == 0 or len(st.session_state.opponent_team) == 0:
            st.warning("Both H2H teams need players.")
        else:
            my_totals_h2h = calculate_totals(st.session_state.h2h_my_team)
            opp_totals_h2h = calculate_totals(st.session_state.opponent_team)
            comparison_df, my_pts, opp_pts = compare_teams(my_totals_h2h, opp_totals_h2h)
            st.dataframe(comparison_df, use_container_width=True, hide_index=True)
            winner = "You Win!" if my_pts > opp_pts else "Opponent Wins!" if opp_pts > my_pts else "Tie!"
            st.metric("FINAL H2H SCORE", f"You {my_pts} – {opp_pts} Opponent", winner)

st.caption("USSSA Fantasy Slow-Pitch • 12-player rosters + 10-player H2H • Live scraping")
